...
namespace ndisapi
{
    ...
    public:
        ...
        explicit static_filters(
            ...
            : log_level_{ log_level }, log_stream_{ log_stream }
        {
        ...
        void remove_filters_if(std::function<bool(const filter<T>&)> predicate)
        {
            ...
            {
                const bool removed = std::visit([&](auto&& arg) -> bool {
                    using FilterType = std::decay_t<decltype(arg)>;
                    if constexpr (std::is_same_v<FilterType, filter<T>>)
                    {
    ...
    private:
        ...
        void print_log(const log_level level, const std::string& message) const noexcept
        {
            ...
            {
                std::osyncstream(log_stream_->get()) << to_string(level) << "\t: " << "[" << std::this_thread::get_id() << "]:[" << typeid(*this).name() << "]: " << message << std::endl;
            }
        }
        ...
        /// </summary>
        std::atomic<log_level> log_level_{ log_level::info };

        /// <summary>
        /// Optional log stream for error logging.
        /// </summary>
